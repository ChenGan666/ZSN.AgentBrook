...
{
    /// <summary>
    /// MCP客户端HTTP实现 - 基于ModelContextProtocol
    /// </summary>
    public class MCPHttpClient : IMCPClient
    {
        ...
        public MCPHttpClient(IMcpClient mcpClient, MCPConfig config, ILogger? logger = null)
        {
            _mcpClient = mcpClient ?? throw new ArgumentNullException(nameof(mcpClient));
            _config = config ?? throw new ArgumentNullException(nameof(config));
            _logger = logger;
            
            LogInfo($"MCP HTTP客户端已初始化，URL: {_config.Url}");
        }
        ...
        public async Task<IList<McpClientTool>> GetToolsAsync()
        {
            try
            {
                LogInfo("Http正在获取可用工具列表...");
                
                var mcpTools = await _mcpClient.ListToolsAsync();
                LogInfo($"Http获取到{mcpTools.Count}个工具:{string.Join(", ", mcpTools.Select(t => t.Name))}");
                return mcpTools;
            }
            catch (Exception ex)
            {
                LogError($"Http获取工具列表异常: {ex.Message}");
                return null;
            }
        }
