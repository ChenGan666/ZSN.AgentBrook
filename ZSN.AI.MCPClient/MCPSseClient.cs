...
{
    /// <summary>
    /// MCP客户端SSE实现 - 基于ModelContextProtocol
    /// </summary>
    public class MCPSseClient : IMCPClient
    {
        ...
        public MCPSseClient(IMcpClient mcpClient, MCPConfig config, ILogger? logger = null)
        {
            _mcpClient = mcpClient ?? throw new ArgumentNullException(nameof(mcpClient));
            _config = config ?? throw new ArgumentNullException(nameof(config));
            _logger = logger;
            
            LogInfo($"MCP SSE客户端已初始化，URL: {_config.Url}");
        }
        ...
        public async Task<IList<McpClientTool>> GetToolsAsync()
        {
            try
            {
                LogInfo("See正在获取可用工具列表...");
                
                var mcpTools = await _mcpClient.ListToolsAsync();
                LogInfo($"See获取到{mcpTools.Count}个工具:{string.Join(", ", mcpTools.Select(t => t.Name))}");

                return mcpTools;
            }
            catch (Exception ex)
            {
                LogError($"See获取工具列表异常: {ex.Message}");
                return null;
            }
        }
        ...
        public async Task<string> InvokeToolAsync(string toolName, string parameters)
        {
            ...
            try
            {
                LogInfo($"=========== [MCP SSE客户端] 开始调用工具 ===========");
                LogInfo($"[MCP SSE客户端] 工具名称: {toolName}");
                LogInfo($"[MCP SSE客户端] 原始参数: {parameters}");
                ...
                if (!string.IsNullOrEmpty(parameters))
                {
                    ...
                    LogInfo($"[MCP SSE客户端] 解析后的参数字典: {JsonSerializer.Serialize(paramDict)}");
                }
                else
                {
                    LogInfo($"[MCP SSE客户端] 无参数传入");
                }
                
                // 调用MCP工具
                LogInfo($"[MCP SSE客户端] 准备调用 _mcpClient.CallToolAsync...");
                var result = await _mcpClient.CallToolAsync(
                    toolName, 
                    paramDict ?? new Dictionary<string, object?>()
                );
                LogInfo($"[MCP SSE客户端] _mcpClient.CallToolAsync 调用完成");
                
                // 处理结果内容
                string responseText = "";
                foreach (var content in result.Content)
                {
                    LogInfo($"[MCP SSE客户端] 结果内容类型: {content.Type}");
                ...
                LogInfo($"[MCP SSE客户端] 工具调用成功: {toolName}");
                LogInfo($"[MCP SSE客户端] 返回结果: {responseText}");
                LogInfo($"=========== [MCP SSE客户端] 工具调用结束 ===========");
                return responseText;
            }
            catch (Exception ex)
            {
