	 ...
	 parameters[3].Value = model.RouteStatus;
        ...
        public MessageReceiveRecordInfo MessageReceiveRecord_DataRowToModel(DataRow row)
        {
            ...
            if (row != null)
            {
				...
				if (row["EventId"] != null )
                {
						model.EventId = row["EventId"].ToString();
                }
                ...
                {
						model.RawPayload = row["RawPayload"].ToString();
                }
				if (row["RoutedWorkflowID"] != null )
                {
						model.RoutedWorkflowID = row["RoutedWorkflowID"].ToString();
                }
				if (row["RoutedTaskID"] != null )
                {
						model.RoutedTaskID = row["RoutedTaskID"].ToString();
                }
				if (row["RouteStatus"] != null )
                {
						model.RouteStatus = int.Parse(row["RouteStatus"].ToString());
                }
