...
_connMultiplexer = ConnectionMultiplexer.Connect(options);
        ...
        static RedisHelper()
        {
            ConnectionString = ConfigHelper.GetString("RedisConnectionString");
            if (ConnectionString == "")
            {
                ConsoleLogHelper.WriteLine($"配置文件中缺失 RedisConnectionString", ConsoleColor.Red);
            }
            else
            {
                try
...
_connMultiplexer = ConnectionMultiplexer.Connect(options);
                ...
                {
                    ConsoleLogHelper.WriteLine($"Redis 连接失败: {e}", ConsoleColor.Red);
                }
            }
        }
        /// <summary>
        ...
        {
            ConsoleLogHelper.WriteLine($"{nameof(ConnMultiplexer_ConfigurationChangedBroadcast)}: {e.EndPoint}");
        }
        /// <summary>
        /// 发生内部错误时(调试用)
        /// </summary>
        ...
        {
            ConsoleLogHelper.WriteLine($"{nameof(ConnMultiplexer_InternalError)}: {e.Exception}");
        }
        /// <summary>
        /// 更改集群时
        /// </summary>
        ...
        {
            ConsoleLogHelper.WriteLine($"{nameof(ConnMultiplexer_HashSlotMoved)}: {nameof(e.OldEndPoint)}-{e.OldEndPoint} To {nameof(e.NewEndPoint)}-{e.NewEndPoint} ");
        }
        /// <summary>
        /// 配置更改时
        /// </summary>
        ...
        {
            ConsoleLogHelper.WriteLine($"{nameof(ConnMultiplexer_ConfigurationChanged)}: {e.EndPoint}");
        }
        /// <summary>
        /// 发生错误时
        /// </summary>
        ...
        {
            ConsoleLogHelper.WriteLine($"{nameof(ConnMultiplexer_ErrorMessage)}: {e.Message}",ConsoleColor.DarkYellow);
        }
        /// <summary>
        /// 物理连接失败时
        /// </summary>
        ...
        {
            ConsoleLogHelper.WriteLine($"{nameof(ConnMultiplexer_ConnectionFailed)}: {e.Exception}",ConsoleColor.Red);
        }
        /// <summary>
        /// 建立物理连接时
        /// </summary>
        ...
        {
            ConsoleLogHelper.WriteLine($"{nameof(ConnMultiplexer_ConnectionRestored)}: {e.Exception}", ConsoleColor.Red);
        }
        #endregion
